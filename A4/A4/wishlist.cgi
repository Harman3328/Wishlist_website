...
def create(user):
    ...
    for item in items:
        prodid = item['id']
        print "<tr><td>"+item['name']+"</td><td><input type='button' value='Remove' id='remove."+str(prodid)+"' onclick='remove()' class='button'</td></tr>"
    print "</table>"
...
if logedIn:
    print "<script src='code4.js'></script>"
    print "<body>"
    ...
    create(logedIn)
    print "<a class='printable' href='printable.cgi'>Print</a></br>"
    print "<a class='printable' href='share.cgi'>Share</a></br>"
    print "<a class='printable' href='shared.cgi'>Shared</a>"
    print "</div>"
    print open('bottom.html','r').read()
    print "</body></html>"
else:
    print "<script src='code2.js'></script>"
    print "<body>"+\
