...
def create(user):
    fs = cgi.FieldStorage()
    if 'from' in fs:
        fromid = fs['from'].value
        fromid = ObjectId(fromid)
        sharedlist = db.A4Users.find_one({'Username':user})['shared']
        for sh in sharedlist:
            if fromid == sh['id']:
                wlist = sh['wishlist']
                print "<table class='product listing' id='mytable'>"
                print "<thead><tr><td><h3>Name</h3></td><td><h3>Cost</h3></td></tr></thead>"
                for prod in wlist:
                    print "<tr><td>"+prod['name']+"</td><td>$"+prod['cost']+"</td></tr>"
                print "</table>"
                break
