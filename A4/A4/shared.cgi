...
def create(user):
    ...
    print "<thead><tr><td><h3>From</h3></td><td><h3>Delete</h3></td></tr></thead>"
    person = db.A4Users.find_one({'Username':user})
    shared = db.A4Users.find_one({'Username':user,'shared':{'$exists':True}})
    if shared:
        slist = person['shared']
        for s in slist:
            print "<tr><td><a href='sharedlist.cgi?from="+str(s['id'])+"' class='sharedlist'>"+s['From']+"</a></td><td><button class='button' onclick='remove()' id='delete."+str(s['id'])+"'>Delete</button></td></tr>"
    print "</table>"
